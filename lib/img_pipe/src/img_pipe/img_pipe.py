...
from img_pipe.plotting.mlab_3D_to_2D import (
    get_world_to_view_matrix,
    get_view_to_display_matrix,
    apply_transform_to_points,
)
from img_pipe.SupplementalFiles import colorLUT
...
# Define the affine transform to go from volume coordinates (as CRS, which is
# the voxel *index* as x,y,z in the 3D volume array) to surface coordinates
# (which is measured in mm from the center of the volume, i.e. FOV). This is
# just the output of: $ mri_info --vox2ras-tkr orig.mgz
affine_vox2tkras = np.array(
    [
        [-1.0, 0.0, 0.0, 128.0],
        [0.0, 0.0, 1.0, -128.0],
        [0.0, -1.0, 0.0, 128.0],
...
# These are the numbers of subcortical structures we want to extract and use, as
# defined in FreesurferLUT.
subcort_roi_numbers_to_use = [
    4,
    5,
    10,
    11,
...
# Dictionary (essentially FreesurferLUT) that maps from a roi number code to its
# label
dict_code_to_label = pickle.load(
    open(
        os.path.join(
            os.path.dirname(__file__), "SupplementalFiles", "FreeSurferLookupTable"
        ),
        "rb",
    )
)
...
class freeCoG:
    ...
    for use in creating brain surface reconstructions, electrode placement, and
    warping.

    To initialize a patient, you must provide the subject ID, hemisphere,
    freesurfer subjects directory, and (optionally) the freesurfer executable
    directory. If these aren't specified they will default to the environment
    ...
    subj_dir : str, optional
               the freesurfer subjects directory (e.g.
               /usr/local/freesurfer/subjects). Default is the $SUBJECTS_DIR
               environmental variable set by Freesurfer.
    ...
    patient_dir : str
        The directory containing data for this particular patient. Usually
        [subj_dir]/[subj]
    hem : str
    ...
    zero_indexed_electrodes : bool
        Whether zero-indexed electrode numbers are used (True) or not (False)
    fs_dir : str
    ...
    def __init__(
        ...
        hem : {'lh', 'rh', 'stereo'}
            The hemisphere of implanation. Can be 'lh', 'rh', or 'stereo'.
        zero_indexed_electrodes: bool, optional
            Whether or not to use zero-indexing for the electrode numbers
            (default: True)
        fs_dir : str, optional
            Path to the freesurfer install (default is $FREESURFER_HOME
            environmental variable)
        subj_dir : str, optional
            Path to the freesurfer subjects (default is $SUBJECTS_DIR
            environmental variable)
        ...
        if self.hem == "stereo":
            ...
            surf_file = os.path.join(
                self.subj_dir, self.subj, "Meshes", self.hem + "_pial_trivert.mat"
            )
        ...
        if not os.path.isdir(os.path.join(self.subj_dir, self.subj)):
            ...
            if ans.upper() == "Y" or ans.upper() == "YES":
                ...
                raise NameError(
                    "No such subject directory as %s" % (os.path.join(subj_dir, subj))
                )
    ...
    def get_recon(self, T3=True, n_jobs=12, use_gpu=False):
        ...
        gpu_flag : {'-use-gpu', ''}
            ...
            otherwise use gpu_flag='' (note that this is deprecated and
            freesurfer doesn't support new CUDA versions)
        """
        # # run recon-all
        ...
        # set up paths for the triangle-vertex files we will generate
        self.pial_surf_file = dict()
        self.pial_surf_file["lh"] = os.path.join(
            self.subj_dir, self.subj, "Meshes", "lh_pial_trivert.mat"
        )
        ...
        # create directory for gyri within labels (we will put a roi label file for each D-K parcel)
    ...
    def check_anatomy_freeview(self):
        ...
        aparc_aseg = os.path.join(
            self.subj_dir, self.subj, "mri", "aparc.a2009s+aseg.mgz"
        )
        os.system(
    ...
    def check_surface_anatomy_freeview(self):
        """Opens Freeview with the pial surfaces and aparc.a2009s labels overalaid."""
        if self.hem == "stereo":
            surf_lh = os.path.join(self.subj_dir, self.subj, "surf", "lh.pial")
            surf_rh = os.path.join(self.subj_dir, self.subj, "surf", "rh.pial")
            os.system(
                "freeview --surface {surf_lh}:annot=aparc.a2009s --surface {surf_rh}:annot=aparc.a2009s --viewport '3d'".format(
                    surf_lh=surf_lh, surf_rh=surf_rh
                )
            )
        else:
            surf = os.path.join(
                self.subj_dir, self.subj, "surf", "{}.pial".format(self.hem)
            )
            os.system(
                "freeview --surface {surf}:annot=aparc.a2009s --viewport '3d'".format(
                    surf=surf
                )
            )
    ...
    def make_dural_surf(self, radius=3, num_iter=30, dilate=0.0):
        ...
        This follows the same procedure as freesurfer goes through during
        recon-all. The only difference is that freesurfer uses white matter, and
        here we want to use smoothed pial surface. We will use marching-cubes
        algorithm to create the mesh (not sure what fs uses).
        ...
        # triangles. Runs mri_pretess to create a connected WM volume
        # (neighboring voxels must have faces in common) and then mri_tessellate
        # to create the surface. The places where the points of the triangles
        # meet are called vertices. Creates the file surf/?h.orig.nofix Note:
        # the topology fixer will create the surface ?h.orig. Finally
        # mris_extract_main_component will remove small surface components, not
        # connected to the main body.

        # note: tessellation of a flat surface means covering of a plane using
        # one or more geometric shapes, called tiles, with no overlaps and no
        # gaps

        from .surface_warping_scripts.make_outer_surf import (
            make_outer_surf,
        )  # From ielu
        ...
        for hem in hems:
            ...
            # First, take pial surface, fill it and make it a volume
            ...
            if os.path.isfile(pial_fill_image):
                ...
                # Create outer surface of the filled volume (this function from ielu does the job)
                make_outer_surf(pial_surf, pial_fill_image, radius, outfile)
                ...
                # mris_smooth will smooth the stuff :)
                os.system(
                    "mris_smooth -nw -n %d %s-main %s" % (num_iter, outfile, dura_surf)
                )

                # mris_expand performs dilatation
                if dilate != 0:
        ...
        # convert the final surface from freesurfer mgz file to a .mat file with
    ...
    def mark_electrodes(self):
        ...
        epicker = os.path.join(
            self.img_pipe_dir, "SupplementalScripts", "electrode_picker.py"
        )
        os.system(
            "python %s %s %s"
            % (epicker, os.path.join(self.subj_dir, self.subj), self.hem)
        )
    ...
    def convert_fsmesh2mlab(self, mesh_name="pial"):
        ...
        are converted into:
            lh_pial_trivert.mat and rh_pial_trivert.mat (for use in python):
                contrains two arrays, "tri" and "vert"
            <sub>_lh_pial.mat and <sub>_rh_pial.mat (for use in MATLAB):
                contains one structure "cortex" that has subfields cortex.tri and cortex.vert
        Both outputs (for matlab and python) have the same information.
        All outputs are saved into the Meshes directory.
        ...
        for h in hems:
            ...
            # use nibabel function to read vertices and triangles from the mgz
            # freesurfer file
            vert, tri = nib.freesurfer.read_geometry(mesh_surf)

            # save mat file for python (triangles and vertices are separate
            # variables)
            out_file = os.path.join(self.mesh_dir, "%s_%s_trivert.mat" % (h, mesh_name))
            scipy.io.savemat(out_file, {"tri": tri, "vert": vert})

            # save mat file for Matlab (triangles and vertices are both fields
            # in a structure called "cortex")
            out_file_struct = os.path.join(
                self.mesh_dir, "%s_%s_%s.mat" % (self.subj, h, mesh_name)
            )
            cortex = {"tri": tri + 1, "vert": vert}
            scipy.io.savemat(out_file_struct, {"cortex": cortex})

        # add path to the data we just made as a freecog class attribute
        if mesh_name == "pial":
            self.pial_surf_file = dict()
            self.pial_surf_file["lh"] = os.path.join(
                self.subj_dir, self.subj, "Meshes", "lh_pial_trivert.mat"
    ...
    def convert_curv(self):
        """Converts freesurfer curvature file to .mat file. """
        hems = ["lh", "rh"]
        if not os.path.isdir(self.mesh_dir):
            print("Making Meshes Directory")
            # Make the Meshes directory in subj_dir if it does not yet exist
        ...
        # Loop through hemispheres for this mesh, create one .mat file for each
        for h in hems:
            print("Making %s curv file" % (h))
            fname_curv = os.path.join(self.surf_dir, h + "." + 'avg_curv')
            # use nibabel function to read vertices and triangles from the mgz
            # freesurfer file
            curv = read_curvature(fname_curv, binary=False)
            # save mat file for python (triangles and vertices are separate
            # variables)
            out_file = os.path.join(self.mesh_dir, "%s_%s_curv.mat" % (self.subj, h))
            scipy.io.savemat(out_file, {"curv": curv})
    ...
    def reg_img(
        ...
        Usually run as patient.reg_img() You can also specify the source
        (usually a CT scan, assumed to be in $SUBJECTS_DIR/subj/CT) and the
        target (usually T1 MRI, assumed to be in $SUBJECTS_DIR/subj/mri)
        ...
        # First prepare the HistogramRegistration object instance and set up
        # parameters. We will use nomalized mutual information because the CT
        # and MRI have very different characteristics (black-white contrast for
        # different tissues is reversed etc.)
        ct_to_mri_reg = HistogramRegistration(
            ctimg, mriimg, similarity="nmi", smooth=smooth, interp=interp
        )

        # fit the regirstration and get affine matrix out of it
        aff = ct_to_mri_reg.optimize(reg_type).as_affine()

        # Put the transformation matrix into an AffineTransform class instance
        # (to keep things nice and tidy). The class needs to know from/to which
        # space the matrix is mapping. The transformation was fitted such that
        # it maps between real-world spaces (i.e. scanner space) of the MRI and
        # CT. So we can copy this info from the CT and MRI nii image files.
        ct_to_mri = AffineTransform(
            ct_cmap.function_range, mri_cmap.function_range, aff
        )

        # now we need to (1) apply the transformation affine and then (2)
        # resample because the mapping was in scanner spaces (world-to-world) so
        # the final position of each value will most likely be between voxels in
        # our required FOV.
        # Notice that this function takes:
        #   -> image to be transformed (CT, we will move this one!)
        #   -> affine for the target image (MRI voxel->scanner)
        #   -> transformation from target.function_range to
        #      image.coordmap.function_range, i.e. 'world-to-world mapping'
        #   -> shape of output array (in target.function_domain, i.e. voxels)
    ...
    def interp_grid(self, nrows=16, ncols=16, grid_basename="hd_grid"):
        ...
        corner_file = os.path.join(
            self.elecs_dir, "individual_elecs", grid_basename + "_corners.mat"
        )
        ...
        for i in np.arange(3):
            elecmatrix[corner_nums[0] : corner_nums[1] + 1, i] = np.linspace(
                elecmatrix[corner_nums[0], i], elecmatrix[corner_nums[1], i], nrows
            )
            elecmatrix[corner_nums[2] : corner_nums[3] + 1, i] = np.linspace(
                elecmatrix[corner_nums[2], i], elecmatrix[corner_nums[3], i], nrows
            )
        ...
        for row in np.arange(nrows):
            for i in np.arange(3):
                elecmatrix[grid[row, :], i] = np.linspace(
                    elecmatrix[row, i],
                    elecmatrix[row + (grid[0, -1] - grid[0, 0]), i],
                    ncols,
                )

        orig_file = os.path.join(
            self.elecs_dir, "individual_elecs", "%s_orig.mat" % (grid_basename)
        )
        scipy.io.savemat(orig_file, {"elecmatrix": elecmatrix})

    def extrap_grid(
        self,
        point_list=[(1, 1), (1, 5), (8, 1)],
        nrows=16,
        ncols=16,
        grid_basename="hd_grid",
    ):
        ...
        points_file = os.path.join(
            self.elecs_dir, "individual_elecs", grid_basename + "_points.mat"
        )
        ...
        for i in np.arange(3):
            linspacecol = np.linspace(
                elecmatrix[points_nums[0], i],
                elecmatrix[points_nums[1], i],
                point_list[1][1],
                retstep=True,
            )
        ...
        for row in range(1, nrows):
            for col in range(1, ncols):
                for i in np.arange(3):
                    elecmatrix[grid[row, col], i] = (
                        elecmatrix[grid[row - 1, col], i] + stepcol[i]
                    )

        grid_file = os.path.join(
            self.elecs_dir, "individual_elecs", grid_basename + "_orig.mat"
        )
    ...
    def project_electrodes(
        ...
        By default, projects the electrodes of a grid based on the mean normal
        vector of the four grid corner electrodes that were manually localized
        from the registered CT. Can also project strips and individual
        electrodes if grid=False (you will be prompted for a projection
        direction).
        ...
        use_mean_normal : bool, optional
            whether to use mean normal vector (mean of the 4 normal vectors from
            the grids corner electrodes) as the projection direction
        surf_type : {'dural','pial','OFC'}, optional
            Which surface to project to.  In the case of the orbitofrontal
            cortex (OFC), it often works better to create a separate ROI of
            those brain structures and project to that, rather than to the pial
            or dural surface
        ...
        grid : bool, optional
            Whether the electrodes to project are from a grid that was
            interpolated using interp_grid()
        convex_hull : bool, optional
            Whether to use the convex hull of the relevant surface or not.
            Often this can result in a smoother looking projection rather than a
            "wavy" looking grid.
        ...
        if grid:
            corners_file = os.path.join(
                self.elecs_dir, "individual_elecs", elecfile_prefix + "_corners.mat"
            )
            elec_corners = scipy.io.loadmat(corners_file)["elecmatrix"]
            elecfile_name = elecfile_prefix + "_orig"
            if not os.path.isdir(
                os.path.join(self.elecs_dir, "individual_elecs", "preproc")
            ):
        ...
        dural_mesh = os.path.join(
            ...
            self.subj,
            "Meshes",
            self.subj + "_" + self.hem + "_dural.mat",
        )
        ...
        if use_mean_normal and grid:
            ...
            normal_vectors = [
                np.cross(grid_vectors[0], grid_vectors[2]),
            ...
            for vec in range(4):
                normal_vectors[vec] = normal_vectors[vec] / np.linalg.norm(
                    normal_vectors[vec]
                )
            ...
            for vec in range(1, 4):
                angle = np.arccos(
                    np.dot(normal_vectors[vec], normal_vectors[vec - 1])
                    / (
                        np.linalg.norm(normal_vectors[vec])
                        * np.linalg.norm(normal_vectors[vec - 1])
                    )
                )
                # flip the normal vector and see if angle between it and the previous normal vector is smaller
                flipped_angle = np.arccos(
                    np.dot((-1.0 * normal_vectors[vec]), normal_vectors[vec - 1])
                    / (
                        np.linalg.norm(normal_vectors[vec])
                        * np.linalg.norm(normal_vectors[vec - 1])
                    )
        ...
        print(
            os.path.join(
                ...
                self.subj,
                "Meshes",
                "%s_%s_trivert.mat" % (self.hem, surf_type),
            )
        )
        ...
        if grid:
            ...
            orig_file = os.path.join(
                self.elecs_dir, "individual_elecs", elecfile_prefix + "_orig.mat"
            )
    ...
    def get_clinical_grid(self, elecfile_prefix="hd_grid"):
        ...
        clingrid = scipy.io.loadmat(
            os.path.join(self.img_pipe_dir, "SupplementalFiles", "clingrid_inds.mat")
        )["inds"].ravel()

        # load in high density grid coordinates
        hd = scipy.io.loadmat(
            os.path.join(self.elecs_dir, "individual_elecs", elecfile_prefix + ".mat")
        )["elecmatrix"]
    ...
    def get_clinical_elecs_all(
        self, elecfile_prefix="TDT_elecs_all", num_grids=1, grid_shortnames=["G"]
    ):
        ...
        print(
            "This only is to be used when the only difference is the density of grids and/or presence of NaNs"
        )
        origelecs = scipy.io.loadmat(
            os.path.join(self.elecs_dir, elecfile_prefix + ".mat")
        )["elecmatrix"]
        ...
        for grid in range(0, num_grids):
            ...
            clingrid = scipy.io.loadmat(
                os.path.join(
                    self.img_pipe_dir, "SupplementalFiles", "clingrid_inds.mat"
                )
            )["inds"].ravel()
            # If we have a grid that is smaller than 256 channels, remove the irrelevant clinical grid indices
            clingrid = clingrid[clingrid < hd.shape[0]]
        ...
        for string in range(0, len(short_label)):
            if masterlist[string] not in (
                ...
                if masterlist[string] not in grid_shortnames:
                    ...
                    for grid in range(0, num_grids):
                        if (
                            masterlist[string] == grid_shortnames[grid]
                            and clinicalgrids[grid].shape[0] > 0
                        ):
        ...
        scipy.io.savemat(
            os.path.join(self.elecs_dir, "clinical_" + elecfile_prefix + ".mat"),
    ...
    def get_subcort(self):
        ...
        # tessellate all subject's freesurfer subcortical segmentations
        print(
            "::: Tesselating freesurfer subcortical segmentations from aseg using aseg2srf... :::"
        )
        subprocess.run(
            [
                os.path.join(self.img_pipe_dir, "SupplementalScripts", "aseg2srf.sh"),
                "-s",
                self.subj,
                "-l",
                " ".join([str(x) for x in subcort_roi_numbers_to_use]),
                "-d",
            ]
        )

        asc_file_list = [f"aseg_{rn:03d}.asc" for rn in subcort_roi_numbers_to_use]
        label_list = [dict_code_to_label[rn] for rn in subcort_roi_numbers_to_use]
        ...
        for fname in srf_list:
            ...
            subprocess.run(
                [
                    "mv",
                    os.path.join(subjAscii_dir, fname),
                    os.path.join(subjAscii_dir, new_fname),
                ]
            )
        ...
        for i in range(len(asc_file_list)):
            fname_asc = os.path.join(subjAscii_dir, asc_file_list[i])
            fname_trivert = os.path.join(subcort_dir, label_list[i])
            self.subcortFs2mlab(fname_asc, fname_trivert)

    def subcortFs2mlab(self, subcort_ascii, nuc):
        ...
        """
        # load ascii file and extract matrix dimensions from header
        with open(subcort_ascii, "r") as f:
            subcort_mat = f.readlines()
        # get rid of comments in header
        subcort_mat.pop(0)
        ...
        subcort_vert = np.array(np.vstack((subcort_vert)), dtype=np.float64)
        ...
        subcort_tri = np.array(np.vstack((subcort_tri)), dtype=np.int64)

        outfile = "%s_subcort_trivert.mat" % (nuc)
        scipy.io.savemat(
            outfile, {"tri": subcort_tri, "vert": subcort_vert}
        )  # save tri/vert matrix

        # convert inds to scipy mat
        subcort_inds = scipy.mat(subcort_inds)
        scipy.io.savemat(
            "%s_subcort_inds.mat" % (nuc), {"inds": subcort_inds}
        )  # save inds
    ...
    def make_elecs_all(self, input_list=None, outfile=None):
        ...
        input_list : list of tuples
            default is None which leads to the interactive prompts to create the
            elecs_all file, but you can input your own list of devices as a list
            of tuples, where each tuple is in the format (short_name, long_name,
            elec_type, filename). The filename is the elecmatrix .mat file of
            the device, and should be in the elecs/individual_elecs folder. If
            you want to add NaN rows, enter
            ('Nan','Nan','Nan',number_of_nan_rows), where in the fourth element
            you specify how many empty rows you'd like to add.
        outfile : str
            the name of the file you want to save to, specify this if not using
        ...
        if input_list != None:
            for device in input_list:
                short_name_prefix, long_name_prefix, elec_type, file_name = (
                    device[0],
                    device[1],
                    device[2],
                    device[3],
                ...
                if (
                    short_name_prefix.lower() == "nan"
                    or long_name_prefix.lower() == "nan"
                    or elec_type.lower() == "nan"
                    or file_name == "nan"
                ):
                    ...
                    short_names.extend(
                        [short_name_prefix + str(i) for i in range(1, num_elecs + 1)]
                    )
                    long_names.extend(
                        [long_name_prefix + str(i) for i in range(1, num_elecs + 1)]
                    )
        ...
        scipy.io.savemat(
            label_outfile, {"eleclabels": eleclabels, "elecmatrix": elecmatrix_all}
        )
    ...
    def label_elecs(
        ...
        Assumes TDT_elecs_all.mat or clinical_elecs_all.mat files. Uses both the
        Desikan-Killiany Atlas and the Destrieux Atlas, as described here:
        https://surfer.nmr.mgh.harvard.edu/fswiki/CorticalParcellation. Note
        that for the labeling of surface electrodes, this method doens't work
        for setting patient.hem = 'stereo'. This is because we want to find the
        closest surface vertex for each electrode, and if the grid/strip is in
        between hemispheres, we don't want to risk assigning the elctrode to the
        opposite hemisphere. This only applies to grids/strips because they are
        insulted on one side (and we know which one it is). There is no such
        issue for depths becuase they record from all directions - so we just
        want to assign to closest voxel(s) label.
        ...
        elec_labels : array-like
            ...
            long name, 'grid'/'depth'/'strip' label, and assigned anatomical
            label.
        """

        if atlas_surf == "desikan-killiany":
            surf_atlas_flag = None
        elif atlas_surf == "destrieux":
            surf_atlas_flag = "--a2009s"
        else:
            surf_atlas_flag = None

        # First, create a .label file for each region in the parcellation. We
        # can save this into a temporary directory subject/label/gyri.
        print(self.subj_dir)
        print(
        ...
        if surf_atlas_flag is not None:
            cmd.append(surf_atlas_flag)
        subprocess.run(cmd)

        print("Loading electrode matrix")
        elecfile = os.path.join(self.elecs_dir, f"{elecfile_prefix}.mat")
        ...
        if (
            ...
            # Prepare a separate array for the original and the new (i.e.
        ...
        label_files = glob.glob(os.path.join(gyri_labels_dir, "{self.hem}.*.label"))
        vert_label = {}
        for label in label_files:
            ...
            with open(label, "r") as f:
                ...
                for v in vertnum:
                    vert_label[np.int64(v)] = label_name.strip()

        trivert_file = os.path.join(self.mesh_dir, f"{self.hem}_pial_trivert.mat")
        ...
        ## Now make a list of the label for each electrode
        ...
        if (
            ...
            # Set depths to an empty string instead of None type
            elec_labels[np.invert(isnotdepth), 3] = ""
        else:
            elec_labels = np.array(elec_labels_notdepth, dtype=np.object)
            print(
                f"Saving electrode labels for surface electrodes to {elecfile_prefix}"
            )

        # If there are depth electrodes, run this part
        if np.any(np.invert(isnotdepth)):
            print(
                " Now doing the depth electrodes ".center(
                    os.get_terminal_size().columns, "*"
                )
            )
            # Get the volume corresponding to the labels from the Destrieux
            # atlas
            ...
            aseg_file = os.path.join(
                self.subj_dir, self.subj, "mri", f"aparc{depth_atlas_nm}+aseg.mgz"
            )
            dat = nib.freesurfer.load(aseg_file)
            aparc_dat = dat.get_data()

            # transform depth electorde coordinates from tkRAS (origin center
            # of FOV) to voxel index (CRS)
            elecmatrix_depths = elecmatrix[np.invert(isnotdepth), :]
            intercept = np.ones(len(elecmatrix_depths))
            elecs_ones = np.column_stack((elecmatrix_depths, intercept))
            elecmatrix_CRS = (
                np.dot(np.linalg.inv(affine_vox2tkras), elecs_ones.transpose())
                .transpose()
                .astype(int)[:, :3]
            )
            # # There is an alternative (better method). We could make a small
            # # sphere around the eletrode coordinate, and check the labels
            # # assigned to voxels that fall into this sphere. Then we could make
            # # a better decision about what label to give this electrode (we
            # # could even take the radius into account etc. in some kind of
            # # probabilistic mapping). To be implemented...
            # xx, yy, zz = np.mgrid[0:aparc_dat.shape[0],
            #                       0:aparc_dat.shape[1],
            #                       0:aparc_dat.shape[2]]
            ...
            # (LUT). This way we can map from the integer code assigned to each
            # voxel in the segmentation MRI to an anatomical label.
            print("Loading lookup table for freesurfer labels")
            lab = self._get_LUT_mapping()

            # Label the electrodes according to the aseg volume
            print("Labeling electrodes...")
            nchans = elecmatrix_CRS.shape[0]
            anatomy = np.empty((nchans,), dtype=np.object)
            for i in np.arange(nchans):
                label_code = aparc_dat[
                    elecmatrix_CRS[i, 0], elecmatrix_CRS[i, 1], elecmatrix_CRS[i, 2]
                ]
                anatomy[i] = lab[label_code]
                print(
                    f"E#{i},\t"
                    f"Vox CRS: {elecmatrix_CRS[i]},\t"
                    f"Label #{label_code:d} = {anatomy[i]}"
                )
        ...
        print(f"Saving electrode labels to {elecfile_prefix}")
        scipy.io.savemat(
            os.path.join(self.elecs_dir, f"{elecfile_prefix}.mat"),
            {
    ...
    def warp_all(
        ...
        elecfile_warped = os.path.join(
            self.elecs_dir, "%s_warped.mat" % (elecfile_prefix)
        )
        elecfile_nearest_warped = os.path.join(
            self.elecs_dir, "%s_nearest_warped.mat" % (elecfile_prefix)
        )
        ...
        if os.path.isfile(elecfile_warped) and not overwrite:
            print(
        ...
        if warp_surface:
            ...
            if not os.path.isfile(
                os.path.join(
                    ...
                    self.subj,
                    "elecs",
                    elecfile_prefix + "_surface_warped.mat",
                )
            ):
            ...
            surface_indices = np.array(
                list(
                    set(np.where(orig_elecs["anatomy"][:, 2] != "depth")[0])
                    & set(
                        np.where(np.all(~np.isnan(orig_elecs["elecmatrix"]), axis=1))[0]
                    )
                ),
                dtype="int64",
            )
            if surface_warps["elecmatrix"].size > 0:
                orig_elecs["elecmatrix"][surface_indices, :] = surface_warps[
                    "elecmatrix"
                ]
    ...
    def get_cvsWarp(self, template="cvs_avg35_inMNI152", openmp_threads=6):
    ...
    def apply_cvsWarp(
        self, elecfile_prefix="TDT_elecs_all", template_brain="cvs_avg35_inMNI152"
    ):
        ...
        VoxCRS = np.dot(np.linalg.inv(affine_vox2tkras), elec.transpose()).transpose()
        ...
        elecmatrix = np.dot(affine_vox2tkras, elec.transpose()).transpose()
        ...
        scipy.io.savemat(
            nearest_warped_matfile, {"elecmatrix": elecmatrix, "anatomy": anatomy}
        )
    ...
    def get_surface_warp(self, basename="TDT_elecs_all", template="cvs_avg35_inMNI152"):
        ...
        if os.path.isfile(elecfile):
            ...
            elecmatrix = scipy.io.loadmat(
                os.path.join(self.elecs_dir, basename + ".mat")
            )["elecmatrix"]
            anatomy = scipy.io.loadmat(os.path.join(self.elecs_dir, basename + ".mat"))[
                "anatomy"
            ]
            ...
            scipy.io.savemat(
                elecfile, {"elecmatrix": np.array(elecs_warped), "anatomy": anatomy}
            )
    ...
    def compute_surface_warp(
        ...
        atlas_file = os.path.join(
            self.subj_dir, template, "Meshes", self.hem + "_pial_trivert.mat"
        )
        ...
        if anatomy is not None:
            surface_indices = np.array(
                list(
                    set(np.where(anatomy[:, 2] != "depth")[0])
        ...
        for c in np.arange(len(surface_indices)):
            ...
            trglabel = os.path.join(
                warped_labels_dir,
                "%s.to.%s.%s" % (self.subj, template, labelname_nopath),
            )
            os.system(
    ...
    def check_depth_warps(
        ...
        nearest_warped_file = os.path.join(
            self.elecs_dir, "%s_nearest_warped.txt" % (elecfile_prefix)
        )
        ...
        cvs_img = nib.freesurfer.load(
            os.path.join(
                self.subj_dir, template, "mri", "aparc" + depth_atlas_nm + "+aseg.mgz"
            )
        )
        ...
        subj_img = nib.freesurfer.load(
            os.path.join(self.mri_dir, "aparc" + depth_atlas_nm + "+aseg.mgz")
        )
    ...
    def apply_xfm(
        ...
        xfm_mat = np.genfromtxt(
            os.path.join(self.patient_dir, xfm_dir, xfm_file),
            skip_header=5,
            delimiter=" ",
            comments=";",
        )
        ...
        if file_type == "elecs":
            ...
            elecs_ones = np.hstack(
                (e["elecmatrix"], np.ones((e["elecmatrix"].shape[0], 1)))
            )
            elecs_xfm = reduce(
                np.dot, [xfm_mat, vox2ras, np.linalg.inv(vox2rastk), elecs_ones.T]
            ).T
            print("Saving transformed coordinates as %s" % (target_file))
            scipy.io.savemat(
                os.path.join(self.patient_dir, target_file),
            ...
            vert_xfm = reduce(
                np.dot, [xfm_mat, vox2ras, np.linalg.inv(vox2rastk), vert_ones.T]
            ).T
            print("Saving transformed coordinates as %s" % (target_file))
            scipy.io.savemat(
                os.path.join(self.patient_dir, target_file),
            ...
            _warning("Please specify file_type = 'surf' or file_type = 'elecs'.")
    ...
    def apply_transform(self, elecfile_prefix, reorient_file):
        ...
        reorient_file = os.path.join(
            self.subj_dir, self.subj, "acpc", reorient_file + ".mat"
        )
        ...
        scipy.io.savemat(
            reoriented_elecfile, {"elecmatrix": elecs_reoriented, "anatomy": anatomy}
        )
        print("Done.")
    ...
    def plot_elec(self, orig_coords, warped_coords, subj_dat, cvs_dat, elec_num, pdf):
        ...
        fs_lut = os.path.join(
            self.img_pipe_dir, "SupplementalFiles", "FreeSurferLUTRGBValues.npy"
        )
        cmap = matplotlib.colors.ListedColormap(np.load(fs_lut)[: cvs_dat.max() + 1, :])

        lookupTable = os.path.join(
            self.img_pipe_dir, "SupplementalFiles", "FreeSurferLookupTable"
        )
        ...
        cvs_vox_CRS = np.array(
            [
                warped_coords[0] - offset,
                warped_coords[1] - offset,
                warped_coords[2] - offset,
            ],
            dtype="int",
        )
        ...
        plt.title(
            "CVS brain "
            + lookup_dict[cvs_dat[cvs_vox_CRS[0], cvs_vox_CRS[1], cvs_vox_CRS[2]]],
            size=20,
        )
        ...
        subj_vox_CRS = np.array(
            [orig_coords[0] - offset, orig_coords[1] - offset, orig_coords[2] - offset],
            dtype="int",
        )
        ...
        plt.title(
            self.subj
            + " "
            + lookup_dict[subj_dat[subj_vox_CRS[0], subj_vox_CRS[1], subj_vox_CRS[2]]],
            size=20,
        )
        ...
        if (
            cvs_dat[cvs_vox_CRS[0], cvs_vox_CRS[1], cvs_vox_CRS[2]]
            != subj_dat[subj_vox_CRS[0], subj_vox_CRS[1], subj_vox_CRS[2]]
        ):
            result = "MISMATCH"
            color = "r"
        else:
    ...
    def get_elecs(
        ...
        """Utility function to get electrode coordinate matrix and labels of all
        electrodes for a defined shaft or anatomical atlas roi. If no shaft_name
        and no roi is passed, it will return coordinates, labels and anatomical
        labels for all electrodes.

        Parameters:
        ----------
        shaft_name : str or list
            name of the electrode shaft(s) (e.g. H') that will be used to match
            as regular expression against electrode labels (default is None,
            which means we don't choose by shaft name)
        roi : str or list
            Which region of interest to load electrodes from. (default is None,
            which means we don't choose by roi name)
        elecfile_prefix : str prefix of the .mat file with the electrode
            coordinates matrix
        atlas : str
            name of the anatomical atlas to use for loading roi labels (e.g.
            'FS' for freesurfer (default), 'Chang' for Hamilton2021).
        whole_shaft : boolean
            If true, and we're searching based on rois, the function returns
            electodes from the whole shaft if it contains at least one electrode
            that matches one of the required rois.
        hem : str | None
            Hemiphere (either 'lh' or 'rh' or None for both)
        all_elecs : boolean
            If True, return all electrodes found in the .mat file irrespective
            of any other arguments.

        Returns:
        -------
        elecmatrix: ndarray
            3D coordintaes of each selected electrode
        eleclabels: list
            Label of each selected electrode
        anatomy: list
            Anatomical label for each electrode
        """
        if atlas == "subcortical":
            e = self._load_elecs(
                elecfile_prefix=elecfile_prefix, atlas="desikan-killiany"
            )
        else:
            e = self._load_elecs(elecfile_prefix=elecfile_prefix, atlas=atlas)

        # Return all elecs
        # ----------------
        # if no shaft name or roi is passed, return all electrodes
        if (shaft_name is None and roi is None and atlas is None) or all_elecs:
            eleclabels = [e["anatomy"][i][0][0] for i in range(len(e["anatomy"]))]
            elecmatrix = e["elecmatrix"]
            anatomy = [e["anatomy"][i][3][0] for i in range(len(e["anatomy"]))]
            return {
        ...
        # Subset only atlas rois
        # ----------------------
        if atlas is not None and roi is None:
            # filter the electrodes so that only regions from the requested
            # atlas are returned
            rois = self._get_available_atlas_labels(atlas=atlas)
            hems = ["lh", "rh"] if hem is None else [hem]
            elecidx = []
            for b in rois:
                for h in hems:
                    full_label = self._prepare_atlas_roi_label(b, atlas, h)
                    (idx,) = np.where(e["anatomy"][:, 3] == full_label)
                    elecidx.extend(idx)
            eleclabels = [e["anatomy"][i][0][0] for i in elecidx]
            elecmatrix = e["elecmatrix"][elecidx]
            anatomy = [e["anatomy"][i][3][0] for i in elecidx]
            if whole_shaft:
                # Find all electrodes from roi-matching shafts (then the
                # code will continue below to find the elecs from shafts
                # that match at least one requested ROI)
                shaft_name = [shft[0] for shft in np.unique(e["anatomy"][elecidx, 1])]
                
        # Search based on roi name(s)
        # ---------------------------
        if roi is not None:
            if type(roi) is not list:
                roi = [roi]
            elecidx = list()
            # prepare list of hemispheres where to look for electrodes
            hems = ["lh", "rh"] if hem is None else [hem]
            # prepare a list of subcortical regions
            subcortical_rois = self.get_rois(atlas="subcortical")
            # go over rois and find matching electrode indices
            for b in roi:
                label_atlas = "subcortical" if b in subcortical_rois else atlas
                for h in hems:
                    full_label = self._prepare_atlas_roi_label(b, label_atlas, h)
                    (idx,) = np.where(e["anatomy"][:, 3] == full_label)
                    elecidx.extend(idx)
            elecidx = np.unique(np.array(elecidx))
            # if you didn't find anything, return empty
            if elecidx.size == 0:
                print("No electodes matching roi %s" % (roi))
                return {"elecmatrix": np.array([]), "eleclabels": [], "anatomy": []}
            if whole_shaft:
                # Find all electrodes from roi-matching shafts (then the
                # code will continue below to find the elecs from shafts
                # that match at least one requested ROI)
                ...
                # If whole_shaft was not requested, we're done.
                eleclabels = [e["anatomy"][i][0][0] for i in elecidx]
                elecmatrix = e["elecmatrix"][elecidx, :]
                anatomy = [e["anatomy"][i][3][0] for i in elecidx]

        # Search based on shaft name(s)
        # -----------------------------
        if shaft_name is not None:
            # if it's not a list, make it
            if type(shaft_name) is not list:
                shaft_name = [shaft_name]
            # prepare labels of all electrodes available for this patient
            eleclabels_all = [e["anatomy"][i][0][0] for i in range(len(e["anatomy"]))]
            # go over the list and check if regexp matches electrode label,
            # if yes append to the output list
            eleclabels = list()
            for shft in shaft_name:
                r = re.compile(shft + r"\d+")
                eleclabels.extend(list(filter(r.match, eleclabels_all)))
            eleclabels = list(np.unique(eleclabels))
            # now find indices of the selected electrode labels
            elecidx = [
                eleclabels_all.index(eleclabels[i]) for i in range(len(eleclabels))
            ]
            # use the indices to get electrode coordinate matrix
            elecmatrix = e["elecmatrix"][elecidx, :]
            # and to get anatomical labels
            anatomy = [e["anatomy"][i][3][0] for i in elecidx]

        return {"elecmatrix": elecmatrix, "eleclabels": eleclabels, "anatomy": anatomy}

    def get_elecs_list(self, eleclabels, elecfile_prefix="TDT_elecs_all"):
        """
        Get coordinates for electrodes from eleclabels list.
        Parameters:
        ----------
        elecfile_prefix : str prefix of the .mat file with the electrode
            coordinates matrix
        eleclabels : list
            list of electrode labels for which coordinates will be returned
        ...
        if os.path.isfile(elecfile):
            ...
            elecidx = list()
            # Search coordinates for each eleclabel
            for label in eleclabels:
                if label in e["anatomy"][:, 0]:
                    elecidx.extend(np.where(e["anatomy"][:, 0] == label)[0])
                else:
                    raise Exception("coordinates for {} not found".format(label))

            elecidx = np.array(elecidx)
            # return matching electrode coordinates
            eleclabels = [e["anatomy"][i][0][0] for i in elecidx]
            elecmatrix = e["elecmatrix"][elecidx, :]

        return elecmatrix, eleclabels
    ...
    def get_surf(self, hem="", roi="pial", template=None):
        ...
        if roi == "pial":
            ...
            cortex = scipy.io.loadmat(
                os.path.join(self.mesh_dir, hem + "_" + roi + "_trivert.mat")
            )
        return cortex
    ...
    class roi:
        ...
        def __init__(
            ...
            """Class defining a region of interest (ROI) mesh. This could be,
            for example, a mesh for the left hippocampus, or a mesh for the
            ...
            name : str
                name of the ROI
            color : tuple
                Tuple for the ROI's color where each value is between 0.0 and
                1.0.
            ...
            gaussian: bool
                specifies how to represent electrodes and their weights on this
                mesh, note that setting gaussian to True means the mesh color
                cannot be specified by the user and will instead use colors from
                the loaded colormap.

            Attributes
            ----------
            name : str color : tuple opacity : float (between 0.0 and 1.0)
            representation : {'surface', 'wireframe'} gaussian : bool
    ...
    def get_rois(
        ...
        -------
        rois : list
            list of available regions of interest (ROIs) for plotting or making
            tri-vert meshes.
        atlas : str
            Name of the anatomical atlas: 'FS' (default), 'chang', 'sma',
            'desikan-killiany', destrieux'
        """
        if from_anatomy:
            if atlas == "subcortical":
                elecfile = os.path.join(
                    self.elecs_dir, f"{elecfile_prefix}_atlas-desikan-killiany.mat"
                )
                rois_subcort = self._get_available_atlas_labels(atlas="subcortical")
            else:
                elecfile = os.path.join(
                    self.elecs_dir, f"{elecfile_prefix}_atlas-{atlas}.mat"
                )
            if os.path.isfile(elecfile):
                e = scipy.io.loadmat(elecfile)
            else:
                raise FileNotFoundError("cannot find the requested elecs file")
            rois = e["anatomy"][:, 3]
            rois = [r[0] for r in rois]
            if clean_labels:
                rois = clean_fs_roi_labels(rois)
            rois = np.unique(rois).tolist()
            if drop_labels:
                rois = drop_useless_roi_labels(rois)
            if atlas == "subcortical":
                rois = [r for r in rois if r in rois_subcort]
        else:
            rois_subcort = self._get_available_atlas_labels(atlas="subcortical")
            if atlas == None or atlas == "desikan-killiany":
                rois_cort = self._get_available_atlas_labels(atlas="desikan-killiany")
                rois = rois_cort + rois_subcort
            elif atlas == "destrieux":
                rois_cort = self._get_available_atlas_labels(atlas="destrieux")
                rois = rois_cort + rois_subcort
            elif atlas == "subcortical":
                rois = rois_subcort
            elif atlas == "chang":
                rois = ["pmHG", "alHG", "PP", "PT", "mSTG", "pSTG"]
            elif atlas == "sma":
                rois = ["SMA", "preSMA"]
        return rois

    def run_annotation2label(self):
        """Create the labels from the Desikan-Killiany and Destrieux atlases.
        Save into respective folders in labels directory."""
        # create gyri labels directory
        atlas_dict = {"desikan-killiany": "aparc", "destrieux": "aparc.a2009s"}
        for atlas_name, atlas_code in atlas_dict.items():
            gyri_labels_dir = os.path.join(
                self.subj_dir, self.subj, "label", atlas_name
            )
            if not os.path.isdir(gyri_labels_dir):
                os.mkdir(gyri_labels_dir)
            else:
                files = os.listdir(gyri_labels_dir)
                for f in files:
                    os.unlink(os.path.join(gyri_labels_dir, f))
            for hem in ["lh", "rh"]:
                subprocess.run(
                    [
                        "mri_annotation2label",
        ...
        # For some reason, mri_annotation2label substitutes "&" isntead of
        # "_and_" in Destrieux atlas. This is weird, because in the colorLUT
        # they clearly use the "_and_" version. Anyway, a quick fix is to
        # just substitute & with _and_ here.
        if atlas_name == "destrieux":
            files = glob.glob(f"{gyri_labels_dir}/*.label")
            for f in files:
                new_fname = re.sub("&", "_and_", f)
                os.rename(
                    os.path.join(gyri_labels_dir, f),
                    os.path.join(gyri_labels_dir, new_fname),
                )
    ...
    def plot_brain(
        ...
        """Plots multiple meshes on one figure. Defaults to plotting both
        hemispheres of the pial surface.

        Parameters
        ----------
        rois : list of roi objects
            (create an roi object like so: hipp_roi = patient.roi(name='lHipp',
            color=(0.5,0.1,0.8), opacity=1.0,
                                   representation='surface', gaussian=True)).
            See get_rois() method for available ROI names.
        elecs : array-like
            [nchans x 3] electrode coordinate matrix
        weights : array-like
            [nchans x 3] weight matrix associated with the electrode coordinate
            matrix
        cmap : str
            String specifying what colormap to use
        elecs_labels: list
            List of electrode names. If passed, label will be plotted next to
            each electrode.
        elecs_anatomy : list
            List of names of anatomical regions where each electrode was
            localized.
        atlas : str
            Name of the atlas where to look for color LUT (must contain
            anatomical regions passed in elecs_anatomy)
        showfig : bool
            whether to show the figure in an interactive window
        screenshot: bool
            whether to save a screenshot and show using matplotlib (usually
            inline a notebook)
        helper_call : bool
            if plot_brain being used as a helper subcall, don't close the mlab
            instance
        vmin : float
        ...
        azimuth : float
            Azimuth for brain view.  By default (if azimuth=None), this will be
            set automatically to the left side for hem='lh', right side for
            hem='rh', or front view for 'pial'
        elevation : float
            Elevation for brain view. Default: 90
        camera_position : tuple
            Value of plotter.camera_position. This is useful for manually tweaking the
            view - first plot the figure and set the view manually, then call
            plotter.camera_position to print the view parameters. You can pass these
            parameters here; it will override azimuth and elevation and recreate
            your manually-set view.
        template : None or str
            Name of the template to use if plotting electrodes on an atlas
            brain. e.g. 'cvs_avg35_inMNI152'

        Returns
        -------
        mesh : mayavi brain mesh points : mayavi electrodes mlab : mayavi mlab
        scene arr : array-like
        ...
        # open a new plotter 
        plotter = ctmr_brain_plot.get_new_plotter()

        all_mesh_vert = []
        
        for roi in rois:
            roi_name, color, opacity, representation, gaussian = (
                roi.name,
                roi.color,
                roi.opacity,
                roi.representation,
            ...
            if roi_name in ("pial", "rh_pial", "lh_pial"):
                ...
                if roi_name in ("pial", "lh_pial"):
                    mesh_data = self.get_surf(hem="lh", template=template)
                if roi_name in ("pial", "rh_pial"):
                    mesh_data = self.get_surf(hem="rh", template=template)
            # otherwise load and plot ROI meshes
            else:
                subcort_dir = os.path.join(self.mesh_dir, "subcortical")
                if os.path.isdir(subcort_dir) and "%s_subcort_trivert.mat" % (
                    roi_name
                ) in os.listdir(subcort_dir):
                    mesh_data = scipy.io.loadmat(
                        os.path.join(subcort_dir, "%s_subcort_trivert.mat" % (roi_name))
                    )
                else:
                    mesh_data = scipy.io.loadmat(
                        os.path.join(self.mesh_dir, "%s_trivert.mat" % (roi_name))
                    )
            # if inflated, load and plot curvature
            if roi_name in ("inflated", "lh_inflated"): 
                curv = self.load_curvature('lh')
            elif roi_name in ("rh_inflated"): 
                curv = self.load_curvature('rh')
            else:
                curv = None
            kwargs.update(
                brain_color=curv, 
            ...
            ctmr_brain_plot.ctmr_gauss_plot(
                mesh_data["tri"], mesh_data["vert"], **kwargs
            )
            # save the vertices        
            all_mesh_vert.append(mesh_data["vert"])

        # show a plane going through the anterior commissure
        if show_ac:
            hemispheres = []
            roi_names = [r.name for r in rois]
            if "rh_pial" in roi_names:
                hemispheres += ["rh"]
            if "lh_pial" in roi_names:
                hemispheres += ["lh"]
            if "rh_pial" not in roi_names and "lh_pial" not in roi_names:
                hemispheres += ["lh", "rh"]
            hemispheres = np.unique(hemispheres)

            for hem in hemispheres:
                # get limits for each axis
                vert = self.get_surf(hem=hem, template=template)["vert"]
                min_lims = vert.min(axis=0)
                max_lims = vert.max(axis=0)
                # generate xyz coordinates for the plane
                x, z = np.mgrid[
                    min_lims[0] : max_lims[0] : 0.1, min_lims[2] : max_lims[2] : 0.1
                ]
                # at AC, y must have this value (transformed from [0,0,0] in
                # scanner volume RAS to [-0.5,  17.5, -18.5] insurface RAS)
                y = np.tile(np.array([17.5]), x.shape)
                # plot the plane
                ctmr_brain_plot.plot_plane(plotter, x, y, z, color=(0.4, 0.4, 0.4), 
                                           opacity=0.6)
        ...
        if not any_gaussian and elecs is not None:
            ...
            if weights is None:
                # if anatomy labels and atlas were passed, find color from the
                # atlas LUT
                if elecs_anatomy is not None and atlas is not None:
                    cmap = colorLUT.get_lut(atlas=atlas)
                    brain_areas = np.unique(elecs_anatomy)
                    good_labels = []
                    color_list = []
                    elec_colors = np.zeros((len(elecs_anatomy), 3))
                    if self.hem == "stereo":
                        label_hem = "lh"
                    else:
                        label_hem = self.hem
                    # Loop through unique brain areas and find the appropriate
                    # color for each brain area from the color LUT dictionary
                    good_labels = []
                    color_list = []
                    for b in brain_areas:
                        ...
                        if b != "NaN":
                            ...
                            if (
                                ...
                                this_label = "ctx-%s-%s" % (label_hem, b)
                            else:
                                this_label = b
                            if this_label != "" and this_label != "NaN":
                                el_color = np.array(cmap[this_label]) / 255.0
                                color_list.append(el_color)
                        elec_indices = np.array(
                            [
                                i
                                for i in range(len(elecs_anatomy))
                                if elecs_anatomy[i] == b
                            ]
                        )
                    ...
                    ctmr_brain_plot.el_add(
                        ...
                        elecs,
                        colors=elec_colors,
                        eleclabels=elecs_labels,
                    )
                # otherwise plot all electrodes with default color
                else:
                    ctmr_brain_plot.el_add(
                        plotter, 
                        elecs, 
                ...
                # If weights were passed, plot them as colormap
                idx_not_nan = np.isnan(weights) == False
                clean_elecs = elecs[idx_not_nan, :]
                clean_weights = weights[idx_not_nan]
                if elecs_labels is not None:
                    clean_elec_labels = [
                        lab for i, lab in enumerate(elecs_labels) if idx_not_nan[i]
                    ]
                else:
                    clean_elec_labels = None
                ...
                ctmr_brain_plot.el_add(
                    ...
                    vmin=vmin,
                    vmax=vmax,
                    eleclabels=clean_elec_labels,
                    show_colorbar=show_colorbar,
                )

                # plot below threshold with constant color
                idx_nan = np.isnan(weights) == True
                clean_elecs = elecs[idx_nan, :]
                clean_weights = weights[idx_nan]
                if elecs_labels is not None:
                    clean_elec_labels = [
                        lab for i, lab in enumerate(elecs_labels) if idx_nan[i]
                    ]
                else:
                    clean_elec_labels = None
        ...
        # # set focal point in the center of the plotted meshes 
        # focalpoint = np.mean(np.vstack(all_mesh_vert), axis=0)
        # plotter.camera.focal_point = focalpoint
        ...
        if camera_location is not None: 
            plotter.camera.position = camera_location
            
        if camera_viewup is not None: 
            plotter.camera_position.viewup = camera_viewup
    ...
    def plot_recon_anatomy(
        ...
        show_numbers: bool, optional
            Whether to show a number next to each electrode (just an arbitrary
            number based on how the elctrodes are ordered in the elecmatrix)
        show_labels: bool, optional
            Whether to show actual label (i.e. name) next to each electrode.
        atlas : str
            Name of the anatomical atlas ('desikan-killiany', 'destrieux', 'chang', 'sma')
        """
        from .plotting import ctmr_brain_plot as ctmr_brain_plot
        ...
        if self.hem == "lh" or self.hem == "rh":
            ...
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                a["lh"]["tri"], a["lh"]["vert"], color=(0.8, 0.8, 0.8), opacity=opacity
            )
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(
                a["rh"]["tri"],
                a["rh"]["vert"],
        ...
        # Prepare list with labels of subcortical ROIs
        subcortical_rois = self.get_rois(atlas="subcortical")
        ...
        for b in brain_areas:
            ...
            # color LUT
            if b[0][0] != "NaN":
                # strip off the 'ctx' and hemisphere information
                full_label = b[0]
                (clean_label,) = clean_fs_roi_labels([full_label])
                # make sure the hemisphere is set to left, so that we don't get
                # separate colors/legend entries for the two hemispheres
                if b[0][0:5] != "Brain" and b[0] != "Unknown":
                    label_atlas = (
                        "subcortical" if clean_label in subcortical_rois else atlas
                    )
                    full_label = self._prepare_atlas_roi_label(
                        clean_label, label_atlas, label_hem
                    )
                if b[0] != "" and b[0] != "NaN":
                    if full_label not in cmap:
                        # in case the label was manually assigned, and not found
                        # in the LUT colormap dictionary
                        _warning(f"cannot find {b} in LUT")
                        el_color = matplotlib.cm.get_cmap("viridis").colors[
                            int(
                                float(np.where(brain_areas == b)[0])
                                / float(len(brain_areas))
                            )
                        ]
                    else:
                        el_color = np.array(cmap[full_label]) / 255.0
                legend_data.update({clean_label: el_color})
            (elec_indices,) = np.where(e["anatomy"][:, 3] == b)
            elec_colors[elec_indices, :] = el_color
        ...
        if show_numbers:
            ...
            elec_numbers = [e["eleclabels"][i] for i in range(e["elecmatrix"].shape[0])]
        else:
            elec_numbers = None
        ...
        if screenshot:
            # this solves the issue in mayavi>4.5.0 (but you have to manually
            # click to stop the interaction, so it's useless for creating
            # animations...)
            mlab.show(stop=True)
            ...
            keys_sorted = sorted(list(legend_data.keys()))
            legend_data_ordered = OrderedDict(
                [(key, legend_data[key]) for key in keys_sorted]
            )
            dummies = [
                plt.gca().plot([], [], ls="-", lw=5, c=c)[0]
                for c in legend_data_ordered.values()
            ]
            plt.gca().legend(dummies, list(legend_data_ordered.keys()), ncol=4)
            plt.tight_layout()
            plt.show()
            plt.savefig(
                os.path.join(self.patient_dir, "%s_anatomy.png" % self.subj), dpi=300
            )
        if showfig:
            mlab.show()
        # else:
    ...
    def plot_erps(
        ...
        if anat_colored:
            anatomy_labels = scipy.io.loadmat(
                os.path.join(self.elecs_dir, elecfile_prefix + ".mat")
            )["anatomy"][:, 3]
                ...
                if b != "NaN":
                    ...
                    if this_label != "":
                        if this_label not in cmap:
                            ...
                            erp_color = np.array(cmap[this_label]) / 255.0
            else:
                erp_color = (1.0, 0.0, 0.0)
                    ...
                    (
                        np.array(
                            [
                                i
                                for i in range(
                                    num_timepoints / 2 - num_timepoints,
                                    num_timepoints / 2,
                                )
                            ]
                        )
                ...
                y_array = (
                    np.array(
                        [
                            i
                            for i in range(
                                num_timepoints / 2 - num_timepoints, num_timepoints / 2
                            )
                        ]
                    )
                ) * time_scale_factor + elec_coord[1]

            x_array = (
                np.array([elec_coord[0] for i in range(num_timepoints)]) + label_offset
            )
    ...
    def plot_recon_anatomy_compare_warped(
        ...
        """This plots two brains, one in native space, one in the template
        space, showing the native space and warped electrodes for ease of
        comparison/quality control.

        Parameters
        ----------
        template : str, optional
            which atlas brain to use (default: 'cvs_avg35_inMNI152').  Must be
            an atlas in the freesurfer subjects directory.
        elecfile_prefix : str, optional
            prefix of the .mat file with the electrode coordinates matrix
        showfig : bool
        ...
        subj_e = self._load_elecs(elecfile_prefix=elecfile_prefix, atlas=atlas)
        template_e = self._load_elecs(
            elecfile_prefix=elecfile_prefix + "_warped", atlas=atlas
        )

        # Offset the brains horizontally so that both can fit into the scene
        ...
        template_brain["vert"][:, 1] = template_brain["vert"][:, 1] + (
            (subj_brain_width + template_brain_width) / 4
        )
        ...
        for b in brain_areas:
            ...
            if b != "NaN" and b[0] != "":
                if b[0] not in cmap:
                    ...
                    el_color = np.array(cmap[b[0]]) / 255.0
                mask = subj_e["anatomy"][:, 3] == b
                ctmr_brain_plot.el_add(
                    np.atleast_2d(subj_e["elecmatrix"][mask, :]),
                    color=tuple(el_color),
                    numbers=elec_numbers[mask],
                )

                ctmr_brain_plot.el_add(
                    np.atleast_2d(template_e["elecmatrix"][mask, :]),
                    color=tuple(el_color),
                    numbers=elec_numbers[mask],
                )
        if self.hem == "lh":
    ...
    def make_roi_mesh(
        self, roi_name, label_list, hem=None, showfig=False, save=True, atlas="gyri"
    ):
        """This function makes a mesh for the cortical ROI you are interested
        in.

        Parameters
        ----------
        roi_name : str
            what you want to call your mesh, note that the hemisphere will be
            prepended to this name
        label_list : list
             A list of labels
        hem : {None, 'lh', 'rh'}
            If None, defaults to the implantation hemisphere.  Otherwise uses
            the hemisphere specified by the user.
        showfig : bool
            Show figure or not.
        save : bool
        ...
        atlas : {None, str}
            If None, the labels are loaded from the label/gyri/ directory. If
            atlas name is passed, it will load from the named atlas subdirectory
            label/<atlas>/.
        Returns
        -------
        roi_mesh : dict
            A dictionary that contains roi_mesh['tri'] and roi_mesh['vert'] and
            can be used in plotting commands for the region of interest
        ...
        # take care of the cortical ROIs
        for label in label_list:
            if label in subcortical_rois:
                # if this is a subcortical region, we'll deal with it later...
                label_list_subcort.append(label)
                _warning(
                    f"{label} is subcortical...trying to use previously generated mesh"
                )
            else:
                this_label = os.path.join(
                    self.patient_dir, "label", atlas, "%s.%s.label" % (hem, label)
                )
                f = open(this_label, "r")
                ...
                verts = np.array(all_lines_float, dtype=np.int64)
        ...
        if vertnums:
            ...
            tri_row_inds = np.sort(
                np.array(
                    list(
                        set(np.where(np.in1d(cortex["tri"][:, 0], vertnums))[0])
                        & set(np.where(np.in1d(cortex["tri"][:, 1], vertnums))[0])
                        & set(np.where(np.in1d(cortex["tri"][:, 2], vertnums))[0])
                    )
                )
            )
        ...
        # now take care of the subcortical ROIs
        for label in label_list_subcort:
            label_full = self._prepare_atlas_roi_label(
                label, atlas="subcortical", hem=hem
            )
            subcort_roi_mesh_path = os.path.join(
                self.mesh_dir, "subcortical", f"{label_full}_subcort_trivert.mat"
            )
            subcort_roi_mesh = scipy.io.loadmat(subcort_roi_mesh_path)
            # reindex vertex indices in triangles
            n_vert = roi_mesh["vert"].shape[0]
            tri_list_reindexed = subcort_roi_mesh["tri"] + n_vert
            # append
            roi_mesh["vert"] = np.append(
                roi_mesh["vert"], subcort_roi_mesh["vert"], axis=0
            )
            roi_mesh["tri"] = np.append(roi_mesh["tri"], tri_list_reindexed, axis=0)
        ...
        if save:
            output_mesh = os.path.join(
                self.mesh_dir, "%s_%s_trivert.mat" % (hem, roi_name)
            )
    ...
    def obj_to_mat(self, hem=None, roi_name="pial"):
        ...
        face = np.asarray(facelines, dtype=np.int64)
        tri = np.zeros((face.shape[0], 3), dtype=np.int64)
    ...
    def convert_bsmesh2mlab(self, mesh_name="pial.cortex"):
        ...
        out_file_struct = os.path.join(
            self.mesh_dir, "bs_%s_%s.mat" % (self.subj, mesh_name)
        )
        scipy.io.savemat(out_file, {"tri": tri, "vert": vert})
        cortex = {"tri": tri + 1, "vert": vert}
        scipy.io.savemat(out_file_struct, {"cortex": cortex})
    ...
    def plot_all_surface_rois(
        ...
        **kwargs,
        ...
        """
        Plots all of the surface rois for a given subject. Uses colors from the
        Freesurfer Color lookup table (LUT) by default.
        ...
        for roi in tqdm(rois):
            if "ctx-" + self.hem + "-" + roi in color_dict:
                ...
                color = np.array(color_dict["ctx-" + self.hem + "-" + roi]) / 255.0
                ctmr_gauss_plot(
                    mesh["tri"], mesh["vert"], color=color, new_fig=False, **kwargs
                )
    ...
    def auto_2D_brain(
        ...
        Some code for projection taken from example_mlab_3D_to_2D by S. Chris
        Colbert <sccolbert@gmail.com>, see:
        ...
        azimuth : float
            Azimuth for brain plot. Normally azimuth=180 for lh, azimuth=0 for
            rh
        elevation : float
        ...
        elecs_2D_file: (optional) None or str
            Filename used when saving electrode position file. If None, filename
            is automatically generated based on view orientation.
        ...
        elecmatrix_2D : array-like
            elecs x 2 matrix of coordinates in 2D that match brain_image and can
            be plotted using matplotlib pyplot instead of mayavi.
        ...
        if brain_file is None:
            brain_file = os.path.join(
                mesh_dir, "brain2D_az%d_el%d%s.png" % (azimuth, elevation, template_nm)
            )
        if elecs_2D_file is None:
            elecs_2D_file = os.path.join(
                self.elecs_dir,
        ...
        if os.path.isfile(brain_file) and force is False:
            ...
            mesh, points, mlab, brain_image, f = self.plot_brain(
                rois=[pial],
                screenshot=True,
                showfig=False,
                helper_call=True,
        ...
        if os.path.isfile(elecs_2D_file) and force is False:
            ...
            view_coords = apply_transform_to_points(
                hmgns_world_coords, combined_transform_mat
            )
    ...
    def animate_scene(
        ...
        start_stop_azimuth=(0, 360),
        start_stop_elevation=(90, 90),
        nframes=200,
        frame_rate=25,
        ...
        Parameters
        ----------
        mlab : mayavi mlab instance movie_name : str
            Name of the movie frame e.g. 'TIMIT_activity'
        mix_type : {'smootherstep', 'smoothstep', 'linear'}
            Easing for the movie.  If 'smootherstep' or 'smoothstep', the
            animation appears to go slowly at the beginning and end, and fast in
            the middle. If 'linear', it evenly rotates with no easing.
        start_stop_azimuth : tuple
            Start and stop azimuth for the plot. Usually you will want this to
            be (0, 360) for 'rh' subjects and (180, 180+360) for 'lh' subjects
        ...
        show_title : bool
            Whether to keep the current mlab figure title on the screen or
            suppress it (Default: False)
    ...
    def load_curvature(self, hem):
        ...
        """
        curv_fpath = f"{self.patient_dir}/surf/{hem}.curv"
        curv = nib.freesurfer.io.read_morph_data(curv_fpath)
        curv = curv / curv.max()  # normalize
        curv[np.where(curv<=0)[0]] = -1
        curv[np.where(curv>0)[0]] = 1
        return curv

    def minimum_dist_to_vol_roi(self, roi, elecfile_prefix="TDT_elecs_all", hem=None):

        # determine what altas the roi is from
        atlas = None
        for atl in ["desikan-killiany", "destrieux", "subcortical"]:
            if roi in self._get_available_atlas_labels(atlas=atl):
                atlas = atl

        # load aseg
        if atlas == "destrieux":
            fname_aseg = "aparc.a2009s+aseg.mgz"
        else:
            fname_aseg = "aparc+aseg.mgz"
        aseg = nib.freesurfer.load(os.path.join(self.mri_dir, fname_aseg))
        aseg_data = aseg.get_fdata()

        roi_ras = []
        hems = [hem] if hem is not None else ["lh", "rh"]
        for h in hems:
            # find the LUT number for the roi
            full_label = self._prepare_atlas_roi_label(roi, atlas, h)
            number_to_label = self._get_LUT_mapping()
            label_to_number = dict([(lab, num) for num, lab in number_to_label.items()])
            try:
                roi_number = label_to_number[full_label]
            except Exception as e:
                _warning("probably didnt find the label in aserg volume")
                raise e
            # find voxels with the roi number
            roi_vox = np.stack(np.where(aseg_data == roi_number), axis=1)
            # transform the voxels to tkRAS
            roi_ras.append(apply_trans(aseg.header.get_vox2ras_tkr(), roi_vox))
        roi_ras = np.vstack(roi_ras)

        # load ALL electrodes
        elecs = self.get_elecs(
            elecfile_prefix=elecfile_prefix, atlas="desikan-killiany", all_elecs=True
        )
        elecmatrix = elecs["elecmatrix"]
        # find the smallest distance to the roi for each electrode
        smallest_dist = np.nan * np.zeros((elecmatrix.shape[0], 1))
        for i_elec, coord in enumerate(elecmatrix):
            smallest_dist[i_elec] = np.min(np.linalg.norm(roi_ras - coord, axis=1))
        return elecs["eleclabels"], smallest_dist

    def _get_LUT_mapping(self):
        # Get the names of these labels using Freesurfer's lookup table (LUT)
        script_dir = Path(__file__).parent
        lut_path = os.path.join(script_dir, "SupplementalFiles", "FreeSurferColorLUT.txt")
        ...
        for row in LUT:
            ...
            if len(row) > 1 and row[0][0] != "#" and row[0][0] != "\\":
                lname = row[1]
                lab[int(row[0])] = lname
        return lab

    def _get_available_atlas_labels(self, atlas="desikan-killiany"):
        """
        Get all possible atlas labels we can use for ROI selection and mesh
        generation. This is done programatically, by looking at the available
        .label files, rather than by having a fixed list. For subcortical
        structures, we call a factored function that loads available labels from
        the aseg mgz file (in an efficient way using lru_cache).

        Arguments
        ---------
        atlas : str
            'desikan-killiany', 'destrieux', 'subcortical'
        """
        if atlas == "subcortical":
            labels = _get_aseg_with_cache(self.mri_dir)
            labels = clean_fs_roi_labels(labels)
        elif atlas in ["desikan-killiany", "destrieux"]:
            labels = glob.glob(
                os.path.join(self.patient_dir, "label", atlas, "*.label")
            )
            labels = [os.path.basename(l) for l in labels]
            # remove suffix
            labels = [os.path.splitext(l)[0] for l in labels]
            # remove hemisphere
            labels = [l[3:] for l in labels]
            labels = clean_fs_roi_labels(labels)
        elif atlas in ["chang", "sma"]:
            labels = drop_useless_roi_labels(list(colorLUT.get_lut(atlas=atlas)))
        return sorted(list(np.unique(labels)))

    def _prepare_atlas_roi_label(self, clean_label, atlas, hem):
        subcortical_hem_dict = {"lh": "Left", "rh": "Right"}
        if atlas == "subcortical":
            full_label = f"{subcortical_hem_dict[hem]}-{clean_label}"
        elif atlas == "desikan-killiany":
            full_label = f"ctx-{hem}-{clean_label}"
        elif atlas == "destrieux":
            full_label = f"ctx_{hem}_{clean_label}"
        else:
            full_label = clean_label
        return full_label

    def _load_elecs(self, elecfile_prefix="TDT_elecs_all", atlas=None):
        if atlas is not None:
            ...
            elecfile = os.path.join(self.elecs_dir, f"{elecfile_prefix}.mat")
        if not os.path.isfile(elecfile):
            raise FileNotFoundError(f"File not found: {elecfile}")
        e = scipy.io.loadmat(elecfile)
        return e
...
def clean_fs_roi_labels(roi_labels):
    prefixes = ["rh", "lh", "ctx", "wm", "Right", "Left", "right", "left"]
    suffixes = ["-", "_"]
    rgx_list = [p + s for p in prefixes for s in suffixes]
    roi_labels_clean = copy.deepcopy(roi_labels)
    for idx, lab in enumerate(roi_labels_clean):
        for expr in rgx_list:
            lab = re.sub(expr, "", lab)
        roi_labels_clean[idx] = lab
    return roi_labels_clean


def drop_useless_roi_labels(roi_labels):
    labels_to_drop = ["Unknown", "unknown", "Cerebral-White-Matter", "Inf-Lat-Vent"]
    roi_labels_clean = [roi for roi in roi_labels if roi not in labels_to_drop]
    return roi_labels_clean


def plot_LUT_legend(ax, atlas, anatomy=None):
    # prepare atlas for legend
    atlas_cmap = colorLUT.get_lut(atlas=atlas)
    # order rois alphabetcally
    sorted_names = sorted(atlas_cmap.keys(), key=lambda x: x.lower())
    # subset only rois that are actually present in the data
    if anatomy:
        sorted_names = [n for n in sorted_names if n in anatomy]
    # clean up the entries
    clean_names = clean_fs_roi_labels(sorted_names)
    ...
    legend_elements = [
        Line2D(
            ...
            marker="o",
            color="w",
            label=clean_names[idx],
            markerfacecolor=np.array(atlas_cmap[roi]) / 255.0,
            markersize=8,
        )
        for idx, roi in enumerate(sorted_names)
    ]
    # avoid repetition
    lines2plot = []
    for idx, roi in enumerate(clean_names):
        if roi not in clean_names[:idx]:
            lines2plot.append(legend_elements[idx])
    ax.legend(
        handles=lines2plot,
        loc="upper right",
        frameon=False,
...
def _method_args_to_parser(method, parser):
    ...
    for arg_number in range(len(method_argnames)):
        if arg_number < required_args:
            ...
            parser.add_argument(
                "--" + method_argnames[arg_number], default=args_values[arg_number]
            )
            ...
            parser.add_argument(
                "--" + method_argnames[arg_number],
                type=type(args_values[arg_number]),
                default=args_values[arg_number],
            )


@functools.lru_cache(maxsize=None)
def _get_aseg_with_cache(mri_dir):
    # At multiple places in freeCoG, we need to get the list of subcotrical
    # strucutres. One way to do this could be to hard-code a list in a text
    # file, or somewhere in the code. But it's also easy to just pull from the
    # aseg volume. The only downside is that the loading takes a lot of time. A
    # solution is to cache the output, but caching a method is a very bad idea
    # (see anthonyexplains). So I factored the function out of the class and it
    # works like a charm ;)
    aseg = nib.freesurfer.load(os.path.join(mri_dir, "aseg.mgz"))
    aseg_codes = np.unique(aseg.get_fdata()).astype(int)
    labels = [dict_code_to_label[code] for code in aseg_codes]
    return labels
