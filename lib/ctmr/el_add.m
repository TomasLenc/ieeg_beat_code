...
% p.addOptional('FaceAlpha',[]);
% p.addOptional('EdgeAlpha',[]);
    ...
    if size(elcol,1)>1 && size(elcol,2)==3
        for i=1:size(els,1)
            ...
            if strcmp(mtype, 'P') ||  strcmp(mtype, 'V') ||  strcmp(mtype, 'N') ||  strcmp(mtype, 'F') || strcmp(mtype,'T')
                ...
                h(i) = plot3(els(i,1),els(i,2),els(i,3),...
                    'Marker', mtype, ...
                    'MarkerFaceColor', elcol(i,:),...
                    'MarkerSize',msize,...
        ...
        if strcmp(mtype, 'P') ||  strcmp(mtype, 'V') ||  strcmp(mtype, 'N') ||  strcmp(mtype, 'F')|| strcmp(mtype,'T')
            ...
            h = plot3(els(:,1),els(:,2),els(:,3), ...
                'Marker',  mtype,...
                'MarkerFaceColor', elcol,...
                'MarkerSize',msize,...
                'MarkerEdgeColor',edgecol,...
                'LineWidth',linewidth,...
                'Linestyle','none');
...
%     h = scatter3(els(:,1),els(:,2),els(:,3), 50, elcol, 'filled')
    
end

% % This doesn't work.....stupid matlab
% if ~isempty(FaceAlpha) || ~isempty(EdgeAlpha)
%     drawnow;
%     markers = h.MarkerHandle;
%     if ~isempty(FaceAlpha)
%        face_col_vec = uint8([elcol FaceAlpha*100])'; 
%        markers.FaceColorData = face_col_vec;
%     end
%     if ~isempty(EdgeAlpha)
%        edge_col_vec = uint8([edgecol EdgeAlpha*100])'; 
%        markers.EdgeColorData = edge_col_vec;
%     end
% end
