function [c_h] = ctmr_gauss_plot(cortex,electrodes,weights,hemi,do_lighting,varargin)
...
%load in colormap
if any(strcmp(varargin,'cmap'))
    cm = varargin{find(strcmp(varargin,'cmap'))+1}; 
else
    % load('loc_colormap')
...
if any(strcmp(varargin,'gsp'))
    gsp = varargin{find(strcmp(varargin,'gsp'))+1}; 
else
...
if any(strcmp(varargin,'mesh_col'))
    c = varargin{find(strcmp(varargin,'mesh_col'))+1}; 
else
...
if any(cortex.tri(:) == 0)
    error('Seems like you are useing python indexing? Add +1 to tris.'); 
end
